...
export function calculateIdv(baseValue, manufactureYear) {
  if (!baseValue || !manufactureYear) return null;
  const age = new Date().getFullYear() - Number(manufactureYear);
  let depreciationPct;
  if (age <= 0) depreciationPct = 0.05;
  else if (age === 1) depreciationPct = 0.15;
  else if (age === 2) depreciationPct = 0.20;
  else if (age === 3) depreciationPct = 0.30;
  else if (age === 4) depreciationPct = 0.40;
  else depreciationPct = 0.50;
  const idv = Number(baseValue) * (1 - depreciationPct);
  return { idv: Math.round(idv), depreciationPct: depreciationPct * 100, age };
}
