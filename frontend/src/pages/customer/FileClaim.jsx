...
export default function FileClaim() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([policyRes, claimRes]) => {
        const activeOnly = policyRes.data.data.content.filter(
          (p) => p.status === "ACTIVE",
        );
  ...
  const selectedPolicy = useMemo(
    () =>
      policies.find((p) => String(p.policyId) === String(form.policyId)) ||
      null,
  ...
  const selectedPolicyClaimSummary = useMemo(() => {
    ...
    const relevantClaims = myClaims.filter(
      (claim) =>
        String(claim.policyId) === String(selectedPolicy.policyId) &&
    ...
    // MOTOR policies are capped by IDV (depreciated value), not the plan's flat coverage
    const effectiveCoverage =
      selectedPolicy.productType === "MOTOR" && selectedPolicy.calculatedIdv
        ? Number(selectedPolicy.calculatedIdv)
        : Number(selectedPolicy.coverageAmount || 0);
    const remaining = Math.max(0, effectiveCoverage - reserved);
    return {
      reserved,
      remaining,
      effectiveCoverage,
  ...
  const validate = () => {
    ...
    if (isBlank(form.claimAmount)) {
      ...
      const coverage = selectedPolicyClaimSummary.effectiveCoverage ?? Number(selectedPolicy.coverageAmount);
    ...
    if (isBlank(form.claimReason)) {
      ...
      !isMeaningfulText(form.claimReason, {
        minLength: 20,
        minWords: 5,
        maxLength: 2000,
      })
      ...
      errors.claimReason =
        "Please describe the incident clearly with at least 5 meaningful words";
    }

    const activeDocuments = documents.filter(
      (d) => d.documentName || d.documentType || d.file,
    );

    if (activeDocuments.length === 0) {
      errors.documents = "Please attach at least one supporting document";
    } else {
      const incomplete = activeDocuments.some(
        (d) => isBlank(d.documentName) || isBlank(d.documentType) || !d.file,
      );
      if (incomplete) {
        errors.documents =
          "Each document row needs a name, type, and an attached file - remove any unfinished rows";
      } else if (
        activeDocuments.some(
          (d) =>
            !isMeaningfulText(d.documentName, {
              minLength: 3,
              minWords: 1,
              maxLength: 150,
            }),
        ...
        errors.documents =
          "Document names must be meaningful (at least 3 characters)";
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(`/customer/claims/${newClaimId}`, {
    ...
    <div>
      <div className="page-header">
        ...
        <p className="page-subtitle">
          Tell us what happened and attach your supporting documents — all in
          one go
        </p>
      </div>
      ...
      <Card>
        ...
        {!policiesLoading && policies.length === 0 && (
          <Alert
            type="error"
            message="You don't have any active policies. A claim can only be filed against an active policy."
          />
        )}
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="form-row">
            <Input
              ...
              helperText={
                selectedPolicy
                  ? `Remaining claimable: ${formatCurrency(selectedPolicyClaimSummary.remaining)} · Reserved in existing claims: ${formatCurrency(selectedPolicyClaimSummary.reserved)}`
            ...
            <Input
              ...
              helperText={
                selectedPolicy?.startDate
                  ? `Policy active since ${formatDate(selectedPolicy.startDate)}`
                  : undefined
              }
          ...
          {selectedPolicy && (
            <div className="safety-warning-card">
              ...
              <p>
                Total coverage is{" "}
                {formatCurrency(selectedPolicyClaimSummary.effectiveCoverage)}
                {selectedPolicy.productType === "MOTOR" &&
                selectedPolicy.calculatedIdv
                  ? " (IDV)"
                  : ""}
                . Existing non-rejected claims reserve{" "}
                {formatCurrency(selectedPolicyClaimSummary.reserved)}, so you
                can currently claim up to{" "}
                {formatCurrency(selectedPolicyClaimSummary.remaining)}.
              </p>
            </div>
          )}
          ...
          <div className="form-field">
            ...
            {fieldErrors.claimReason ? (
              ...
              <span className="field-hint">
                {form.claimReason.length}/2000 characters
              </span>
            )}
          </div>
          ...
          <p className="form-section-hint">
            Attach the actual files (hospital bill, invoice, photo, etc.) that
            support this claim — PDF, JPG, or PNG.
          </p>
          {fieldErrors.documents && (
            <Alert
              type="error"
              message={fieldErrors.documents}
              onClose={() => clearFieldError("documents")}
            />
          )}
          ...
          {documents.map((doc, index) => (
            <div key={index} className="document-row-full">
              ...
              {documents.length === 1 &&
                (doc.documentName || doc.documentType || doc.file) && (
                  <button
                    type="button"
                    className="link-btn"
                    onClick={() => clearDocumentRow(index)}
                  >
                    Clear
                  </button>
                )}
          ...
          {documents.length < MAX_DOCUMENT_ROWS ? (
            ...
            <span className="field-hint">
              Maximum of {MAX_DOCUMENT_ROWS} documents per claim
            </span>
          )}
