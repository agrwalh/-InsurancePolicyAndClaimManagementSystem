...
import { Car, Plane, HeartPulse, Shield } from "lucide-react";
import {
  formatCurrency,
  formatDate,
  formatLabel,
...
function getAnnualPaymentWindow(policy) {
  ...
  if (policy.status === "LAPSED") {
    return {
      allowed: true,
      message:
        "⚠️ This policy has lapsed due to a missed payment. Pay now to revive coverage immediately.",
    };
  }
  ...
  const windowDays = paymentWindowDays[policy.premiumFrequency] ?? 5;
...
export default function MyPolicies() {
  ...
  const { fieldErrors, setFieldError, clearFieldError, clearAll } =
    useFormErrors();
  ...
  const perYearSplits = (freq) => ({ MONTHLY: 12, QUARTERLY: 4, HALF_YEARLY: 2, ANNUAL: 1 }[freq] || 1);
  const effectiveAnnual = (p) => {
    if (!p) return 0;
    if (p.premiumType === "ANNUAL") return Number(p.installmentAmount || 0) * perYearSplits(p.premiumFrequency);
    // one-time policy
    return Number(p.installmentAmount ?? p.premiumAmount ?? 0);
  };
  const isHealthAdjusted = (p) => {
    if (!p || p.productType !== "HEALTH") return false;
    const eff = effectiveAnnual(p);
    const base = Number(p.premiumAmount || 0);
    return Math.round(eff) !== Math.round(base);
  };
  ...
  const handlePayWithRazorpay = async (policy) => {
    ...
    try {
      ...
      setError(
        extractErrorMessage(err, "Could not start payment. Please try again."),
      );
    } finally {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((p) =>
      ...
      includesCI(p.productType, q) ||
      includesCI(p.status, q),
    );
        ...
        <div className="table-wrap">
          ...
          <div style={{ display: "flex", gap: "0.75rem", alignItems: "center", padding: "0.75rem" }}>
            <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
              {["ALL", "ACTIVE", "PENDING_PAYMENT", "LAPSED"].map((s) => (
                <button
                  key={s}
                  type="button"
            ...
            <div style={{ marginLeft: "auto", minWidth: 220, maxWidth: 320 }}>
              <Input
                placeholder="Search by policy no, plan, status…"
                value={search}
          ...
          <table className="data-table">
            ...
            <tbody>
              {filteredRows.map((policy) => {
                const annualWindow = getAnnualPaymentWindow(policy);
                  const Icon = policy.productType === "MOTOR" ? Car : policy.productType === "TRAVEL" ? Plane : policy.productType === "HEALTH" ? HeartPulse : Shield;
                  const effAnnual = effectiveAnnual(policy);
                  const baseAnnual = Number(policy.premiumAmount || 0);
                return (
                  <tr key={policy.policyId}>
                    <td>{policy.policyNumber}</td>
                      <td>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <span className="tile-icon" style={{ width: 28, height: 28 }}>{<Icon size={16} />}</span>
                          <div>
                            <div style={{ fontWeight: 600 }}>{policy.planName}</div>
                            <div className="field-hint">{policy.productType?.toLowerCase?.()}</div>
                          </div>
                        </div>
                      </td>
                    <td>
                    ...
                    <td>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <span>
                            ...
                            {policy.premiumType === "ANNUAL" ? ` / ${(policy.premiumFrequency || "").toLowerCase()}` : ""}
                          </span>
                          {isHealthAdjusted(policy) && (
                            <span className="badge badge-amber">Adjusted</span>
                          )}
                        </div>
                        {policy.premiumType === "ANNUAL" && (
                          <div className="field-hint">
                            Base: {formatCurrency(baseAnnual)} / year · Effective: {formatCurrency(effAnnual)} / year
                          </div>
                        )}
                    </td>
                    <td>
                      {policy.premiumType === "ANNUAL" ? (
                        <div>
                          <div>{`${policy.premiumsPaid ?? 0}/${policy.totalInstallments ?? "-"}`}</div>
                          {typeof policy.remainingInstallments === "number" && (
                            <div className="field-hint">{policy.remainingInstallments} remaining</div>
                          )}
                        </div>
                      ) : (
                        "-"
        ...
        <div className="modal-overlay" onClick={() => setPayingPolicy(null)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <Input
              ...
              helperText={
                payingPolicy.premiumType === "ANNUAL"
                  ? getAnnualPaymentWindow(payingPolicy).message ||
                    "This is your EMI installment amount"
            ...
            <div style={{ marginTop: "1rem" }}>
              <p className="field-hint" style={{ marginBottom: "0.75rem" }}>
                You'll be redirected to a secure Razorpay checkout to complete
                this payment via UPI, Card, or Net Banking.
              </p>
              {payingPolicy.premiumType === "ANNUAL" && (
                <dl className="detail-list" style={{ marginBottom: "0.6rem" }}>
                  <div>
                    <dt>Base annual</dt>
                    <dd>{formatCurrency(Number(payingPolicy.premiumAmount || 0))}</dd>
                  </div>
                  <div>
                    <dt>Effective annual</dt>
                    <dd>{formatCurrency(effectiveAnnual(payingPolicy))}</dd>
                  </div>
                </dl>
              )}
